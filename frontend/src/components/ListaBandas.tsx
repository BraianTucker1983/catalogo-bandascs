...
import { client } from '../lib/api'; // Cliente Hono RPC
  ...
  useEffect(() => {
    ...
    async function cargarBandas() {
      try {
        ...
        if (!cancelado) {
          setBandas((data as BandaResumen[]) || []);
        }
      } catch (err) {
        if (!cancelado) {
        ...
        <main className="flex-1 p-6 md:p-8 overflow-y-auto">
          {cargando ? (
            ...
            <div className="text-center py-12 px-4 bg-destructive/10 border border-destructive/30 rounded-2xl space-y-3 max-w-md mx-auto">
              <p className="text-destructive font-bold text-sm">Error al consultar el servidor</p>
