...
export default function EditarBanda({ onVolver }: EditarBandaProps) {
  ...
  // Gestión de Memoria para URLs de Blob
  ...
  // --- CARGA INICIAL DE DATOS BASADA EN LA SESIÓN DE SUPABASE ---
  useEffect(() => {
    cargarPerfilBanda();
  }, []);
  ...
  const cargarPerfilBanda = async () => {
    ...
    try {
      ...
      if (userError || !user) {
        throw new Error('Debes iniciar sesión para editar tu banda.');
      }

      // 2. Obtener banda del usuario (asumiendo columna user_id en la tabla bandas)
      ...
      if (!bandaData) {
        setMensaje({ tipo: 'error', texto: 'No se encontró un perfil de banda asociado a tu cuenta.' });
        return;
      }

      // 3. Cargar Integrantes y Canciones en paralelo
      ...
      if (bandaData.url_portada) setPortadaPreviewUrl(bandaData.url_portada);
      ...
      setMensaje({ tipo: 'error', texto: err.message || 'Error al obtener la información de la banda.' });
    } finally {
      setCargando(false);
    }
  };
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            ...
            resolve(new File([blob], `${nombreSinExt}.webp`, { type: 'image/webp' }));
          },
          'image/webp',
          calidad
        );
  ...
  const manejarSeleccionPortada = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const archivoOptimizado = await optimizarEConvertirAWebP(e.target.files[0]);
      revocarObjectUrl(portadaPreviewUrl);
      setPortada(archivoOptimizado);
      setPortadaPreviewUrl(crearObjectUrl(archivoOptimizado));
    } catch (error) {
  ...
  const actualizarIntegrante = (index: number, campo: keyof IntegranteInput, valor: any) => {
    setIntegrantes((prev) => prev.map((item, i) => (i === index ? { ...item, [campo]: valor } : item)));
  };

  const manejarFotoIntegrante = async (index: number, e: ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files || e.target.files.length === 0) return;
    try {
      const archivoOptimizado = await optimizarEConvertirAWebP(e.target.files[0], 800, 0.75);
  ...
  const actualizarCancion = (index: number, campo: keyof CancionInput, valor: string) => {
    setCanciones((prev) => prev.map((item, i) => (i === index ? { ...item, [campo]: valor } : item)));
  };
  ...
  const handleGuardarCambios = async (e: FormEvent) => {
    ...
    try {
      ...
      if (portada) {
        ...
        if (uploadError) throw new Error(`Error en portada: ${uploadError.message}`);
      ...
      if (errorBanda) throw new Error(`Error en datos generales: ${errorBanda.message}`);
      ...
      const idsABorrarIntegrantes = idsIntegrantesOriginales.filter((id) => !idsActualesIntegrantes.includes(id));

      if (idsABorrarIntegrantes.length > 0) {
        await supabase.from('integrantes').delete().in('id', idsABorrarIntegrantes);
      }

      for (const int of integrantesValidos) {
        let urlFotoIntegrante = int.fotoPreviewUrl || null;

        if (int.foto) {
          const rutaFoto = `integrantes/${Date.now()}_${int.foto.name}`;
          const { data: uploadFoto, error: errorFoto } = await supabase.storage
            .from('Bandas')
            .upload(rutaFoto, int.foto, { contentType: 'image/webp', upsert: true });

          if (errorFoto) throw new Error(`Error en foto integrante: ${errorFoto.message}`);
        ...
        const payload = {
          ...
          instrumento: int.instrumento.trim() || null,
          rol: int.instrumento.trim() || null,
      ...
      const idsABorrarCanciones = idsCancionesOriginales.filter((id) => !idsActualesCanciones.includes(id));

      if (idsABorrarCanciones.length > 0) {
        await supabase.from('canciones').delete().in('id', idsABorrarCanciones);
      }

      for (const can of cancionesValidas) {
        const payload = {
          banda_id: bandaId,
          titulo: can.titulo.trim(),
          spotify_embed_url: can.spotify_embed_url.trim() || null,
          youtube_embed_url: can.youtube_embed_url.trim() || null,
        };

        if (can.id !== undefined) {
          await supabase.from('canciones').update(payload).eq('id', can.id);
        } else {
          await supabase.from('canciones').insert([payload]);
        }
      }

      await cargarPerfilBanda();
  ...
  if (cargando) {
    return (
      <div className="flex items-center justify-center min-h-[400px] text-slate-400 gap-3">
        ...
        <span>Cargando perfil de la banda...</span>
      </div>
    );
  }
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-800/50 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                    required
                  />
                </div>
              ...
              <div>
                ...
                <div className="flex gap-3 items-center">
                  ...
                  />
                  <span className="text-xs text-slate-400">Color distintivo de tu ficha.</span>
                </div>
              </div>
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-700 border-dashed rounded-xl cursor-pointer bg-slate-800/30 hover:bg-slate-800/60 transition group">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p className="text-xs text-slate-400">
                        <span className="font-semibold text-slate-300">Seleccionar nueva portada</span>
                      </p>
                    </div>
                  ...
                  {portadaPreviewUrl && (
                    <div className="flex items-center justify-between p-3 bg-slate-800/40 rounded-xl border border-slate-700/60">
                      <div className="flex items-center gap-3">
                        <img src={portadaPreviewUrl} alt="Portada" className="w-16 h-12 object-cover rounded-lg" />
                        <span className="text-xs text-slate-300">Imagen de portada seleccionada</span>
                      </div>
                      <button type="button" onClick={eliminarPortada} className="p-2 text-slate-400 hover:text-rose-400 transition">
                        <X className="w-5 h-5" />
                      </button>
                    </div>
                  )}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Biografía / Historia</label>
                <textarea
                  rows={4}
                  value={historia}
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium transition"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {integrantes.map((int, index) => (
                  <div key={int.id || int.tempId} className="p-4 bg-slate-800/40 border border-slate-700/60 rounded-xl flex gap-4 items-start relative">
                    <button type="button" onClick={() => eliminarIntegrante(index)} className="absolute top-3 right-3 text-slate-500 hover:text-rose-400">
                      <Trash2 className="w-4 h-4" />
                    </button>
                    <label className="relative w-16 h-16 rounded-full bg-slate-700 flex-shrink-0 flex items-center justify-center cursor-pointer overflow-hidden border border-slate-600">
                      {int.fotoPreviewUrl ? (
                        <img src={int.fotoPreviewUrl} alt="Integrante" className="w-full h-full object-cover" />
                      ) : (
                        <ImageIcon className="w-6 h-6 text-slate-400" />
                      )}
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => manejarFotoIntegrante(index, e)} />
                    </label>
                    <div className="flex-1 space-y-2 pr-6">
                      <input
                        ...
                        className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm text-slate-100"
                      />
                      <input
                        type="text"
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="space-y-4">
                {canciones.map((can, index) => (
                  <div key={can.id || can.tempId} className="p-4 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-3">
                    <div className="flex justify-between items-center gap-3">
                      <input
                        ...
                        className="flex-1 bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm font-semibold text-slate-100"
                      />
                      <button type="button" onClick={() => eliminarCancion(index)} className="p-2 text-slate-500 hover:text-rose-400">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <input
                        type="text"
                        placeholder="Embed / Link Spotify"
                        value={can.spotify_embed_url}
                        onChange={(e) => actualizarCancion(index, 'spotify_embed_url', e.target.value)}
                        className="w-full bg-slate-800/80 border border-slate-700/80 rounded px-3 py-1.5 text-xs text-slate-200"
                      />
                      <input
                        type="text"
            ...
            <div>
              {pasoActual < 3 ? (
                ...
                <button
                  type="submit"
                  disabled={guardando}
                  className="flex items-center gap-2 px-6 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 text-white rounded-lg text-sm font-semibold transition shadow-lg"
                >
                  {guardando ? (
                    <>
